...
interface UserResponse {
  ...
  export function Account() {
    ...
    const { register: searchRegister, handleSubmit: handleSearchSubmit } = useForm<SearchFormInputs>();
    const { register: updateRegister, handleSubmit: handleUpdateSubmit } = useForm<UpdateProfileInputs>();
    ...
    const updateUserInfoMutation = useMutation({
      mutationFn: async (data: UpdateProfileInputs) => {
        const response = await api.patch(`/usuario/${userProfile?.id}`, data);
        return response.data;
      },
      onSuccess: () => {
    ...
    const updateAvatarMutation = useMutation({
      mutationFn: async (file: File) => {
        ...
        const response = await api.patch(`/usuario/avatar/${userProfile?.id}`, formData, {
          headers: {
      ...
      onError: (error) => {
        ...
        toast.error("Erro ao atualizar o avatar do usuário.");
      },
    });
    ...
    const onUpdateUserInfo: SubmitHandler<UpdateProfileInputs> = (data) => {
      ...
      <div className="container mx-auto p-6">
        <Card className="w-full max-w-md mx-auto">
          ...
          <CardContent>
            <form onSubmit={handleSearchSubmit(onSearch)} className="space-y-4">
              <div className="flex space-x-2">
                <Input
                  ...
                  {...searchRegister("email", { required: true })}
                />
            ...
            {userProfile && (
              <div className="mt-6 space-y-4">
                ...
                <form onSubmit={handleUpdateSubmit(onUpdateUserInfo)} className="space-y-4">
                  <Input
                    type="text"
                ...
                <div className="space-y-2">
                  <Label htmlFor="avatar">Atualizar Avatar</Label>
                  <Input
                    id="avatar"
                    type="file"
                    accept="image/*"
          ...
          <CardFooter className="text-sm text-gray-500">
            Clique nos botões correspondentes para salvar as alterações.
          </CardFooter>
        </Card>
      </div>
    );
